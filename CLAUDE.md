...
The Hermes team also does the engineering: its engineering-lead, backend-dev, frontend-dev, mobile-dev, devops-engineer, ui-designer, design-reviewer, qa-engineer, security-auditor and code-reviewer profiles build on branches named `hermes/<issue>-<slug>` and open pull requests labelled `hermes` and `claude-verify`. Claude's job on those is verification: `hermes-verifier` reviews the PR against the issue, the standards and CI, then labels `verified` or `rework`; the owner merges. The agents in this repository build only on issues labelled `claude-code`, and that label is added only when the owner explicitly tells the Hermes chief to send a task to Claude.
