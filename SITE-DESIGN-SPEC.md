...
> 4. **Matches en Activiteit hergebruiken bestaande, gedeelde routes** in plaats van een eigen matcheslijst: Matches las `GET /api/v1/admin/pipeline?candidate_id=` (BV1) en toonde vacature, fase en laatst gewijzigd; Activiteit leest `GET /api/v1/admin/activities?subject_type=candidate&subject_id=` zoals §7.3.6(b) voor de klantdrawer al doet. Er is geen aparte "matches"-route voor één kandidaat en de pipeline-rijvorm is inhoudelijk hetzelfde ding. **Bijgewerkt met de bouw van §7.3.4:** de tab Matches bestaat niet meer. De tab Pipeline (§7.3.4) heeft hem vervangen, niet ernaast gezet: twee tabs op dezelfde route was verwarrend, en Pipeline toont dezelfde drie kolommen plus de fasewisselaar en de historietijdlijn. Activiteit blijft ongewijzigd.
...
> **Status, september 2026: gebouwd**, als gedeelde functies in `website/admin/js/admin.js` (`Admin.loadPipelineTab()` en de functies eromheen), afgenomen door `website/admin/js/sections/candidates.js` (kandidaatdrawer) en `website/admin/js/sections/clients.js` (klantdrawer). De labelmap staat in `js/labels.js` (`pipelinefase`, plus `AdminLabels.pipelineStages` voor de spec-volgorde). Payloads nagelezen tegen `talent-os/backend/routers/admin.py` (regel 1451-1564: `GET /pipeline`, `PATCH /pipeline/{entry_id}/stage`, `GET /pipeline/{entry_id}/history`), `models/schemas.py` (regel 828-839 `PipelineStage`/`PIPELINE_STAGES`, regel 1233-1245 `PipelineStageUpdate`/`PipelineStageHistoryItem`) en `core/pipeline.py` (`PIPELINE_ROW_SQL`, `project_pipeline_rows`). Tien as-builtafwijkingen:
> 1. **De tab Matches in de kandidaatdrawer is vervangen, niet ernaast blijven bestaan** (§7.3.2 afwijking 4 was nog "Matches leest GET /admin/pipeline?candidate_id= en toont vacature, fase en laatst gewijzigd"; dat is nu de tab Pipeline zelf). Twee tabs op dezelfde route was verwarrend voor een beheerder ("welke van de twee is actueel?"), en Pipeline is functioneel een superset van Matches (dezelfde drie kolommen, plus de fasewisselaar en de historietijdlijn). `_candidateTabs` in `candidates.js` heeft dus geen `matches`-item meer; `loadCandidateMatchesTab()` is vervangen door `loadCandidatePipelineTab()`.
> 2. **`showCandidateName`, geen letterlijk spec-detail maar nodig om de twee drawers hetzelfde component te laten delen.** De kandidaatdrawer toont, zoals het wireframe, alleen "Vacature: `<titel>` (#`<id>`)" per kaart -- er is maar één kandidaat in beeld, de eigen naam er nog eens bij zetten voegt niets toe. De klantdrawer kan wél meerdere kandidaten door elkaar tonen (één `client_id`, verschillende `candidate_id`'s over verschillende vacatures), en zou zonder naam onmogelijk te lezen zijn; die kop wordt daar `<naam> · Vacature: <titel> (#<id>)`. Eén functie (`Admin.renderPipelineTab()`), een `opts.showCandidateName`-vlag per aanroeper.
> 3. **De historie komt van de backend oplopend binnen** (`ORDER BY h.changed_at`, geen `DESC` -- admin.py regel 1561) en wordt in `loadPipelineHistory()` client-side aflopend gesorteerd (id als tiebreak bij een gelijke timestamp) vóór het renderen, want de tijdlijn hoort nieuwste boven te tonen (§7.3.4 hierboven, wireframe).
> 4. **Eén toast bij een geslaagde fasewijziging** ("Fase bijgewerkt"), die de spectekst niet met zoveel woorden vraagt maar die overeenkomt met hoe elke andere inline-opslag in een drawertab dit doet (bijvoorbeeld `saveClientInfo()` in `clients.js`, "Opgeslagen"). Zonder toast zou de enige zichtbare bevestiging de (na een fractie van een seconde) verversende historie zijn, en dat is een zwakker signaal dan bij vergelijkbare acties elders in het paneel.
> 5. **Het client-side wachtwoord tegen de "(bestaande waarde)"-ontsnappingsklep zit vóór de aanroep, niet alleen in de reactie op een 422.** `changePipelineStage()` weigert een PATCH te versturen wanneer de geselecteerde waarde niet één van de zeven canonieke fasen is (`AdminLabels.pipelineStages.includes(newStage)`), met een eigen inline-melding. Dat geldt ook voor een lege waarde (`stage: null`, security-auditor LOW): de kolom heeft geen `NOT NULL`, en zonder deze klep zou de browser bij een lege `<select>` stil de eerste optie ('sourced') kiezen, waardoor "Fase wijzigen" zonder enige keuze van de beheerder een PATCH zou sturen. Een lege fase krijgt daarom dezelfde ontsnappingsklep als een onbekende (`value=""`, label "(leeg) (bestaande waarde)").
> 6. **De vangnet-lezing van de ontsnappingsklep is bijgesteld: hij is geen verwachte hoofdroute, ook niet vóór `VALIDATE CONSTRAINT`.** Migratie 043 normaliseert bij de eigen deploy al elke bestaande rij naar een van de zeven waarden en zet de CHECK meteen daarna als `NOT VALID` -- en `NOT VALID` slaat alleen de eenmalige tabelscan over, niet de handhaving zelf: elke INSERT en elke UPDATE op `stage` wordt al vanaf die deploy geweigerd zodra de waarde buiten de zeven valt (zie het bestand zelf). Na migratie 043 kan `stage` op een bestaande rij dus normaal gesproken geen onbekende, niet-lege waarde meer krijgen. De klep in `admin.js` blijft niettemin nodig als vangnet voor twee gevallen die de constraint niet kan afdwingen: `stage: null` (geen `NOT NULL`, punt 5 hierboven) en een rij die buiten de reguliere schrijfpaden om is gewijzigd (een handmatige UPDATE, een toekomstige migratie). `PIPELINE_STAGE_VALIDATED` blijft dezelfde vlag, met dezelfde betekenis (open tot de eigenaar `VALIDATE CONSTRAINT` draait); alleen de reden waarom hij nog aanstaat is preciezer dan de eerste versie van dit bestand beweerde.
> 7. **De 44px-tikdoelregel (§8.x.0) dekte de offcanvas nog niet voor formuliervelden.** De bestaande regel had `.modal .form-select, .modal .form-control` maar niet de `.offcanvas`-tegenhangers; de fasewisselaar staat in de drawer zelf, niet in een modal. `admin.css` kreeg `.offcanvas .form-select, .offcanvas .form-control` erbij, in dezelfde `@media (max-width: 600px)`-regel.
> 8. **Geen paginering binnen de tab.** `GET /admin/pipeline` levert `total`/`limit`/`offset`, maar deze tab vraagt in één keer het maximum (`limit=200`) op en toont alles: een kandidaat of opdrachtgever met meer dan 200 pipeline-entries is in de praktijk niet te verwachten, en een tweede paginacomponent binnen een drawertab (naast de al bestaande accordeon) zou de kaart drukker maken dan de spec voorziet.
> 9. **`entry.updated_at` staat altijd op de kaart, onafhankelijk van de historie** (code-reviewer LOW). De oude tab Matches toonde "Bijgewerkt" uit `updated_at`; Pipeline leidt "Laatst gewijzigd" in plaats daarvan af uit de historie, en die kan ontbreken (geen historie) of mislukken (§7.3.4 hierboven, de foutstaat). Zonder een aparte regel stond er dan nergens meer een datum bij een kaart die er wel degelijk een heeft. `pipelineEntryBody()` rendert een vaste `.a-meta`-regel "Bijgewerkt: `<entry.updated_at>`", los van of de historie laadt, leeg is, of faalt.
> 10. **Alleen de nieuwste entry haalt zijn historie meteen op; de rest pas bij het openklappen** (code-reviewer LOW). `renderPipelineTab()` laadde bij de eerste bouw de historie van elke entry tegelijk, ook een dichtgeklapte kaart in de accordeon. Sinds deze ronde krijgt elke `<details>` behalve de nieuwste een `toggle`-listener die de historie één keer ophaalt zodra hij voor het eerst opengaat (hetzelfde patroon als `#retentionTableDetails` in `retention.js`), zodat een drawer met veel entries niet meteen evenveel historie-aanroepen doet als er kaarten zijn.
>
> **Eén reviewronde na de eerste bouw (security-auditor, code-reviewer, design-reviewer), allemaal verwerkt.**
> - **Kruisdrawer-schrijffout (code-reviewer, HIGH).** Een gesloten drawer blijft in de DOM, verborgen (§7.2b), en een pipeline-entry kan in zowel de kandidaat- als de klantdrawer voorkomen (dezelfde `entry_id`, want het is dezelfde rij). Elke DOM-lookup in `admin.js` ging via het kale `document.getElementById()`/`document.querySelector()`, dat bij een dubbel id altijd het EERSTE element in de documentvolgorde teruggeeft -- na "klantdrawer openen > Pipeline > sluiten > kandidaatdrawer openen > Pipeline" werkte de kandidaatdrawer op de verborgen kopie uit de klantdrawer: "Fase wijzigen" stuurde niets (de select las de stilstaande waarde van de verborgen kopie), en de historie bleef op zijn laadskelet staan. Elke functie die zo'n element opzoekt (`changePipelineStage()`, `pipelineEntryAlert()`, `loadPipelineHistory()`, `renderPipelineHistory()`) neemt sindsdien een `root`-parameter aan (de `.a-tabpane` van de drawer die de aanroep deed, doorgegeven via `el.closest('.a-tabpane')` in de twee `data-action`-registraties) en zoekt daarbinnen. Regressietest in `scripts/admin_sections_check.py`: klantdrawer Pipeline openen en sluiten, dan de kandidaatdrawer openen en aantonen dat `#candidateDrawer #pipelineHistoryWrap_604` geen laadskelet toont en dat "Fase wijzigen" daadwerkelijk een PATCH stuurt, vóór welke `page.reload()` dan ook in de suite (die zou het verborgen restant wegvegen zonder de bug te bewijzen).
> - **Mobiele lege ruimte tussen select en knop (design-reviewer).** `.a-pipeline-controls .form-group { flex: 1 1 220px }` is een breedtebasis voor de rijrichting op breed scherm; in de `@media (max-width: 600px)`-regel schakelt `.a-pipeline-controls` naar `flex-direction: column`, en dezelfde 220px werd daar een HOOGTEbasis -- 165px lege ruimte tussen de select en de knop op 390px, in beide drawers. `.a-pipeline-controls .form-group { flex: 0 0 auto; }` in die media query laat het formuliergroepje zijn eigen veldhoogte houden.
>
> **Tweede reviewronde (security-auditor, code-reviewer), allemaal verwerkt.**
> - **Historie-race zonder volgnummer (security-auditor LOW).** `loadPipelineHistory()` had geen manier om te weten of zijn eigen `GET .../history`-respons nog wel de nieuwste aanroep vertegenwoordigde: een trage, eerder gestarte aanroep die laat binnenkwam kon zo de kaart terugzetten op een oudere regel, ook al had een geslaagde `PATCH` intussen al een verse herlading getoond. `wrap._pipelineHistorySeq` (een oplopend volgnummer per `pipelineHistoryWrap_<id>`-element) legt vast welke aanroep de nieuwste is; een respons waarvan het volgnummer niet meer overeenkomt met het huidige wordt genegeerd, in zowel het succes- als het foutpad. Getoetst met een deterministische promise-race op `Admin.loadPipelineHistory()` zelf (`Auth.fetch` tijdelijk vervangen, de nieuwste aanroep eerst laten binnenkomen en renderen, dan pas de oudste) in plaats van via echte netwerktiming: Playwrights routedispatch bleek in deze omgeving geserialiseerd, dus een vastgehouden of vertraagde HTTP-respons kreeg de bedoelde uit-volgorde-aankomst niet betrouwbaar voor elkaar.
> - **De ontsnappingsklep voor een lege fase moet ook na `VALIDATE CONSTRAINT` blijven staan (code-reviewer LOW).** `pipelineStageOptions()` liet de klep vervallen zodra `PIPELINE_STAGE_VALIDATED` op `true` staat, ook voor `stage: null`. Dat is onjuist: een CHECK-constraint laat `NULL` altijd door (SQL-standaardgedrag), dus `VALIDATE CONSTRAINT` bewijst niets over een lege `stage` en de browser zou zonder klep stil de eerste optie ('sourced') kiezen. De voorwaarde is nu `!known && (!PIPELINE_STAGE_VALIDATED || currentStage == null)`: de klep voor een ONBEKENDE, niet-lege waarde vervalt nog altijd na `VALIDATE`, de klep voor een LEGE waarde niet. `PIPELINE_STAGE_VALIDATED` staat sindsdien ook als `Admin.PIPELINE_STAGE_VALIDATED` (naast de module-const die de bron blijft), zodat de test de vlag kan forceren zonder het bestand aan te raken.
> - **Twee races in `scripts/admin_sections_check.py` (code-reviewer FIX FIRST).** (a) Een klik op "Vastleggen (presentatie, intrekken)" volgde direct op het sluiten van de presentatiemodal, zonder te wachten tot de achtergrondherlading (`switchCandidateTab(..., {force: true})`) na de geslaagde presentatie-PATCH klaar was -- drie van zes runs klikten zo op een knop die halverwege die herlading uit de DOM verdween. Dezelfde `wait_until`-vorm als bij de talentpool-herlading ernaast, nu ook hier. (b) "Lijst genereren" wachtte alleen tot de POST de stub bereikte, niet tot de eigen achtergrondherlading (`refreshRetention()`, twee extra `GET`'s plus een her-render van `#retentionBody`) ook klaar was; de knop zelf gaat pas ná die hele keten weer aan, dus dat is het juiste signaal om op te wachten in plaats van de aanroepteller. Twee van zes runs stierven daarna met een traceback op een kale `eval_on_selector('#retentionApproveModal .modal-body')`, want als de goedkeurmodal (die door deze race soms niet binnen 6s opende) er niet stond, gooide Playwright een fout in plaats van een regel in `failures` te zetten; beide metingen op die selector staan nu achter een `query_selector`-guard.
>
> Bewaakt door `scripts/admin_sections_check.py`: zeven gestubde pipeline-entries op candidate_id=1482/client_id=1 (waarvan zes op de kandidaat, voor de accordeon-met-zes-entries-test, en alle zeven op de opdrachtgever, plus een tweede kandidaat voor de naam-in-de-kop), het exacte zeven-opties-in-spec-volgorde-contract van de select, de "(bestaande waarde)"-optie voor zowel een onbekende als een lege fase die geen van beide ooit verstuurd wordt (ook met `PIPELINE_STAGE_VALIDATED` geforceerd op `true`), direct na het renderen van de tab dat de historie van precies de nieuwste entry is opgehaald en van geen enkele andere (lui laden rechtstreeks bewaakt), één PATCH met de juiste payload gevolgd door precies één verse `GET .../history`-aanroep (niet lokaal aangevuld, niet alleen "de tekst staat er al ergens"), een 422 gevolgd door een 500 gevolgd door een geslaagde poging op dezelfde entry (optimistische update, terugdraaien, dan alsnog slagen), de drie actor-varianten in de historie (genoemd, "Gebruiker #`<id>`", "Onbekend") en het `(nieuw)`-label bij `from_stage: null`, "Bijgewerkt" dat blijft staan zonder historie en mét een historiefout, twaalf historie-items met "Toon alles", een entry waarvan de historieroute altijd 500 teruggeeft terwijl de fasewisselaar zelf bruikbaar blijft, de historie-race zonder volgnummer, en de kruisdrawer-regressie hierboven.
...
> **Status, september 2026: gebouwd**, als `website/admin/js/sections/gdpr.js` plus een lege `<section id="section-gdpr">` en het sidebar-item in `website/admin/index.html`. Payloads nagelezen tegen `talent-os/backend/routers/gdpr.py` (regels 340-749 `erase_person()`, 771-856 `AdminEraseRequest`/`admin_erase_person()`, 861-923 suppressielijst), niet tegen deze spec alleen. Vijf as-builtafwijkingen:
> 1. **`ui.modal`'s getypte bevestiging vergeleek tot nu toe altijd hoofdlettergevoelig.** Dit scherm is de enige afnemer van een bevestiging die geen vaste tekenreeks is maar het ingevoerde e-mailadres zelf, en de backend zelf vergelijkt na `strip()`/`lower()` (`privacy.normalize_email`). `ui.js` kreeg er daarom twee paneelbrede opties bij: `confirmCaseInsensitive` (vergelijkt na `trim()` en `toLowerCase()`) en `confirmInputType` (`'email'` in plaats van het standaard `'text'`), plus `handle.confirmValue()` om te lezen wat er precies getypt is -- het bevestigingsveld heeft geen door de aanroeper gekozen id. Elke andere destructieve modal in het paneel (APPROVE, een plaatsings-ID) blijft ongewijzigd hoofdlettergevoelig.
> 2. **De tweede bevestiging (`confirm_admin_or_self`) is dezelfde modal, geen tweede modal**, met een tweede knop die pas na de 409 zichtbaar wordt (`hidden`, en tot de checkbox is aangevinkt op slot via `data-gsp-lock`, hetzelfde patroon als de categoriebrede goedkeuring in `retention.js`). `ui.modal` kent maar één rode rol (`danger`) per modal en die is al bezet door de eerste knop ("Wissen (definitief)"); de tweede knop wordt daarom als `primary` (goud) aangemaakt en pas bij het zichtbaar worden client-side van klasse gewisseld naar `.btn-outline-danger`, zodat een handeling die beheerderstoegang verwijdert nooit in het goud van een gewone primaire actie staat. De eerste knop gaat na de 409 zelf op slot (`data-gsp-lock`): opnieuw klikken zou alleen dezelfde weigering herhalen.
> 3. **Domeinfilter is client-side, alleen op de geladen pagina.** `GET /admin/suppression` kent geen domeinparameter (alleen `limit`/`offset`, gelezen op regel 906-923); het zoekveld filtert daarom uitsluitend de al opgehaalde pagina van 100 en zegt dat er expliciet bij ("Geen adressen op deze pagina met dit domein. De suppressielijst kent geen zoekopdracht over de hele lijst.") in plaats van een lege staat te suggereren die op de hele verzameling lijkt te gelden.
> 4. **De opsomming "wat er gebeurt" in de wismodal is twaalf regels, niet vijf.** Het wireframe noemt vijf categorieën ter illustratie; de daadwerkelijke `.a-panel` somt letterlijk elke tabel op die `erase_person()` raakt (kandidaatgegevens, CV-bestanden, portalaccount, pushmeldingen, pipeline-notities, vacature-alertverzendingen, prospectgegevens, outreach, quiz/contact, eerdere AVG-verzoeken, auditlog, suppressielijst), met per rij anonimiseren dan wel verwijderen, precies zoals de functie het doet.
> 5. **"Domein groot, hash klein" op 390px komt uit bestaande klassen, niet uit een nieuwe.** De suppressietabel is een gewone `.a-cardlist` met `data-label` per cel; het domein krijgt `.a-cell-name` (600 gewicht, wit) en de hash `.a-num.a-soft` (mono, gedempt) -- dezelfde twee klassen die elders al identiteit versus metadata onderscheiden, dus geen sectie-eigen CSS nodig.
> 6. **`ui.modal`'s `close()` liet het paneel-element zelf gevuld achter** (alleen de `show`-klasse ging eraf), dus een getypt e-mailadres bleef als tekst en als `input.value` in de verborgen DOM staan, ook na Annuleren en na een geslaagde wissing. Security-audit-reparatie, paneelbreed: `close()` doet nu `mount(el, '')` direct na `instance.hide()`/`fallbackHide(el)` -- op beide paden zonder animatie (geen `.fade`-klasse, §7.2c/§7.2b), dus na die aanroep heeft Bootstrap zijn eigen `hidden.bs.modal`/`hidden.bs.offcanvas`-afhandeling al synchroon afgerond. Elke volgende opening van hetzelfde element gaat weer door `panel()`, dat vóór `instance.show()` altijd een volledig nieuwe `mount(el, ...)` doet, dus dit maakt een latere hervulling nooit onvolledig. Tweede reparatie in dezelfde ronde: het bevestigingsveld werd na de 409 (punt 2 hierboven) op slot gezet (`disabled`) en `submitGdprErase()` leest de getypte bevestiging nu maar één keer in (bij de eerste aanroep) en hergebruikt die voor de tweede, zodat de twee aanroepen voor hetzelfde adres nooit een verschillende `confirm` kunnen dragen.
> 7. **Design-review FIX FIRST, drie punten.** (a) De tweede stap (punt 2 hierboven) verschijnt onderaan een modalbody die al de twaalfregelige opsomming (punt 4) draagt; op een korter scherm, of nadat die opsomming de ruimte al vult, viel hij buiten beeld en leek de klik op de eerste knop niets te doen. `gdprShowSecondStep()` doet nu `el.scrollIntoView({ block: 'start' })` op het net gemounte blok zelf, hetzelfde patroon (en dezelfde optie) als `candidates.js`' `candidateAlert()` en `placements.js`' `placementAlert()`. (b) De datum in de suppressielijst droeg `.a-num--date` (font-size-lg, punt 5): prima in een gewone tabelrij, maar op 390px staat die datum als enige meerregelige waarde naast domein en reden op dezelfde kaart en oogt te zwaar. `admin.css` kreeg één regel naar het bestaande `.a-cardlist td[data-label="..."]`-patroon (zoals "Onderwerp"/"Ontbrekend signaal" al doen): `[data-label="Toegevoegd op"]` valt terug op `--font-size-sm`/gewicht 400 onder 768px, geen nieuwe klasse. (c) `submitGdprErase()` zette bij een geslaagde wissing `handle.setBusy(true)` nooit meer terug: `gdprRenderEraseResult()` zet de knoppen alleen `disabled`+`data-gsp-lock`, niet de labeltekst, dus de spinner uit `setBusy(true)` bleef na "Volledig verwerkt" oneindig draaien. Het succespad roept nu eerst `handle.setBusy(false)` (herstelt de knoptekst) vóór `gdprRenderEraseResult()` (zet daarna zelf `disabled`+`gspLock`), zodat de eindstaat de eigen knoptekst toont, niet de spinner.
>
> Bewaakt door `scripts/admin_sections_check.py` (de hele sectie tegen gestubde routes) en `scripts/admin_pagination_check.py` (120 rijen naast de paginagrootte van 100, met genoemde faalregels in plaats van een vaste `wait_for_timeout` na de sectiewissel en na de pagina-2-klik -- zelfde `wait_until`-patroon als `admin_sections_check.py`).
...
| `#placements` | Plaatsingen | Plaatsingenlijst (§7.3.3), in de sidebargroep Jobs onder Opdrachtgevers |
| `#gdpr` | AVG | Wissen (art. 17) en suppressielijst (§7.3.5), in de sidebargroep System, onder Bewaartermijnen |

`#retention` (Bewaartermijnen, §7.3.1), `#placements` (Plaatsingen, §7.3.3) en `#gdpr` (AVG, §7.3.5) zijn gebouwd; `#retention` en `#gdpr` staan in de sidebargroep System, boven Settings.
...
| Schil | `website/admin/admin.css` | het `--tblr-*`-blok uit §7.1.2, de vier `--ink-*`-tokens, compatklassen, 29 `.a-*`-utilityklassen uit §7.1.3 (inmiddels 46 in totaal, plus een aparte reeks panelklassen zoals `.a-modal`/`.a-drawer`/`.a-panel-footer`) plus de vier `.text-*-ink`, gedocumenteerd bovenaan het bestand |
...
| Labels | `website/admin/js/labels.js` | zestien enum→labelmaps (geteld in het bestand: kandidaatstatus, erkendReferent, contactRol, activiteit, leadCategorie, dienstlijn, retentiestatus, retentiecategorie, retentieactie, retentieonderwerp, grondslag, toestemmingsomvang, plaatsingstype, plaatsingstatus, afrekenbasis, kostentype), Nederlands, sleutels uit de backend. Wordt in de §7.2e-pass vervangen door `js/status-map.js` met `{label, tone}` per waarde |
...
**Kandidaatdrawer en Toestemmingen (§7.3.2).** `website/admin/js/sections/candidates.js` registreert de kandidaatdrawer (Profiel, Pipeline, Activiteit, Toestemmingen -- Pipeline heette bij het bouwen van deze sectie nog Matches, zie §7.3.4) op `ui.drawer`/`ui.tabs`, en de referral-intake en de twee wijzigmodals op `ui.modal`, zoals het Opdrachtgevers-detailpaneel dat al deed. Verschillende dingen zijn paneelbreed, niet alleen voor deze sectie: `.a-skel-block` (een nieuwe `.a-`-klasse, een vlak `--navy-600`-blok voor de "drie vlakke blokken"-laadstaat van een drawertab uit §7.2b, die er nog nergens stond), de twee `--tblr-form-invalid-*`-tokens in `admin.css`, die `.invalid-feedback`/`.is-invalid` van Tablers hardcoded `#d63939` naar `var(--ink-error)` zetten, en twee reparaties uit de review op deze sectie die eveneens paneelbreed gelden: `.form-group label:not(.form-check-label)`/`.form-group input:not([type="checkbox"]):not([type="radio"])` (zodat een radiogroep of checkbox in een `<fieldset class="form-group">` niet de volle breedte en de hoofdletterstijl van een gewoon tekstveld krijgt), en de uitbreiding van de 44px-tikdoelregel op 390px met `#section-candidates .btn` en `.offcanvas .btn, .modal .btn, .modal .form-select, .modal .form-control`. Twee nieuwe labelmaps in `js/labels.js` (`grondslag`, `toestemmingsomvang`), letterlijk uit de tabellen in §7.2e.

**Tab Pipeline in beide drawers (§7.3.4).** Gedeelde functies in `website/admin/js/admin.js` (`Admin.loadPipelineTab()` en eromheen), afgenomen door zowel `candidates.js` (kandidaatdrawer, verving de tab Matches) als `clients.js` (klantdrawer, nieuwe zesde tab tussen Vacatures en Notities/Activiteit). Twee nieuwe paneelbrede klassen: `.a-timeline` (de verticale historietijdlijn, hergebruikbaar voor de Activiteitentab van §7.3.6(b), die vandaag nog een platte tabel is) en `.a-pipeline-controls` (select plus knop op de kaart). De 44px-tikdoelregel op 390px kreeg `.offcanvas .form-select, .offcanvas .form-control` erbij: de bestaande regel dekte alleen `.modal .form-select`/`.form-control`, en de fasewisselaar staat in de drawer zelf. Eén nieuwe labelmap in `js/labels.js` (`pipelinefase`, plus `AdminLabels.pipelineStages` voor de spec-volgorde van de select). `scripts/admin_sections_check.py` bewaakt de accordeon, de zeven-opties-select, de ontsnappingsklep, de PATCH-payload en -terugval, de drie actor-varianten en "Toon alles" (§7.3.4 as-built).

**Sectie Plaatsingen (§7.3.3).** `website/admin/js/sections/placements.js` registreert `#placements` met zijn eigen `skeletonHtml()`, zoals Bewaartermijnen dat deed; `index.html` draagt alleen het lege `<section>` en het sidebar-item. Vier dingen zijn paneelbreed (`website/admin/js/ui.js`, `admin.css`), niet alleen voor deze sectie, en kwamen allemaal met deze PR mee na twee mislukte tussenstappen op ditzelfde probleem:

1. **Backdrop-opacity.** De echte Bootstrap-backdrop draagt nooit `.a-backdrop` (die klasse hoort alleen bij het eigen vangnet zonder Bootstrap-JS), dus de merkkleur (`--navy-950` op `.72`) stond op een selector die de echte backdrop nooit raakte: Tablers eigen `.modal-backdrop.show`/`.offcanvas-backdrop.show` (specificiteit 0,2,0) won altijd van een kale `.modal-backdrop` (0,1,0). Gemeten: `.24` vóór de fix, `.72` erna, op zowel de modal- als de offcanvas-backdrop.
2. **`offcanvas-bottom` op 390px, plus een `footer`-optie op `ui.drawer`.** Een drawer kiest bij het openen `offcanvas-bottom` (`max-height: 92vh`, decoratieve `aria-hidden`-sleepgreep `.a-drawer__grip`) op maximaal 767px in plaats van altijd `offcanvas-end`; boven die breedte blijft `offcanvas-end`. `ui.drawer({ footer: { primary, secondary } })` geeft een drawer een eigen voettekst (maximaal twee acties, secundair links en primair rechts, `keepOpen` daar standaard aan omdat een drawer niet hoort te sluiten na een footeractie zoals een modal dat na "OK" wel doet) -- de plaatsingendrawer gebruikt die voor "Bewerken". Gemeten op 390x844: klasse `offcanvas a-drawer offcanvas-bottom a-drawer--bottom show`, hoogte 776–870px (92vh van 844 is 776,48px; varieert met de tabinhoud), lege band onder de voettekst 1px.
3. **De voettekst van elke modal en drawer staat buiten de scrollende inhoud.** Twee eerdere pogingen loste dit niet op: eerst `position: sticky` binnen `.modal-body`/`.offcanvas-body` zelf (die bleef bij `scrollTop 0` over de eerste velden liggen, want sticky-in-flow lost geen overlap op, het verplaatst hem alleen), daarna een via JS gemeten `padding-bottom` op de scrollcontainer (die na een tabwissel of resize uit de pas kon raken, met een lege band of alsnog overlap als gevolg). De uiteindelijke vorm is een gewone flexbox-broer: `.a-panel-footer` staat naast `.modal-body` binnen `.modal-content` (dat `modal-dialog-scrollable` al een flexkolom met een eigen max-height geeft) respectievelijk naast `.offcanvas-body` binnen `.offcanvas` (zelf al een flexkolom); geen `position: sticky`, geen gemeten padding, geen resize-listener meer. Gemeten bij `scrollTop 0` op vier oppervlakken, 1440x1000 en 390x844: aanmaakmodal, verwijdermodal, plaatsingendrawer en de bewaartermijnen-goedkeurmodal tonen elk hun onderste veld zonder overlap (`document.elementFromPoint` op het midden van dat veld geeft het veld zelf terug), en de bewaartermijnenmodal op 1440 heeft `scrollHeight <= clientHeight` (geen scroll meer nodig voor die korte modal). De lege band onder de knoppen is nu altijd hetzij 0–1px (een drawer of een fullscreen modal op 390, die de volle hoogte vult) hetzij Bootstraps eigen verticale centrering van een korte modal (204px op de verwijdermodal 1440, 111,5px op de bewaartermijnenmodal 1440) -- geen apart geïntroduceerde marge, dus geen aparte regel om te bewaken.
4. **`.form-group label:not(.form-check-label)`-labelstijl.** Niet meer uppercase mono maar Plex Sans `--font-size-sm` gewicht 500 in `--navy-100` (§7.2d regel 617). Gemeten op "Nieuwe vacature": 12px/600/`#7FA0C9`/uppercase+0,06em vóór, 14px/500/`#C5D6EB`/geen tekst-transformatie erna.

Twee dingen zijn sectie-eigen: `.a-num--tab` (`font-variant-numeric: tabular-nums`, modifier van `.a-num`, apart van `.a-num--date`, voor elke tabel- of paneelwaarde die op zijn cijfers moet uitlijnen zonder de leesmaat van een datum te delen), `.a-notes` (`white-space: pre-wrap`, voor elk paneel dat vrije tekst met regeleindes toont zonder inline `style=`) en `.a-hide-cardlist` (verbergt een kolom of de rijverwijderknop onder 768px; alleen deze kaartlijst toont op 390px minder kolommen dan op desktop, en de verwijderknop is met opzet niet bereikbaar op 390 -- verwijderen kan daar alleen vanuit de drawer niet, en dat is bewust: zie §7.3.3 as-built hieronder). Plus de uitbreiding van de 44px-tikdoelregel op 390px met `#section-placements .btn` naast de al bestaande `#section-retention`/`#section-candidates`. Vier labelmaps in `js/labels.js` (`plaatsingstype`, `plaatsingstatus`, `afrekenbasis`, `kostentype`), letterlijk uit `_PLACEMENT_TYPES`/`_PLACEMENT_STATUSES`/`_BILLING_BASES`/`_FEE_TYPES` in `models/schemas.py`.

**As-builtaanvulling op §7.3.3.** "Bewerken" staat in de panelfooter (`.a-panel-footer`) van de plaatsingendrawer, buiten de scrollcontainer (punt 2 hierboven), niet in de tab Financieel: het bewerkmodal wijzigt ook Overzicht-velden (start-/einddatum, notities), niet alleen financiële velden. Verwijderen is alleen bereikbaar vanuit de rij in de lijst (de rijknop met het prullenbakicoon); die knop is `.a-hide-cardlist`. Op 390 is verwijderen niet mogelijk, ook niet vanuit de drawer; dat kan alleen in de brede weergave, en dat is bewust. Een bedrag met een duizendtalscheiding zoals `1.234,56` wordt afgewezen met de foutmelding "Gebruik één decimaalteken": de parser accepteert komma óf punt als decimaalteken, nooit allebei tegelijk in hetzelfde getal.

**Sectie AVG (§7.3.5).** `website/admin/js/sections/gdpr.js` registreert `#gdpr` met zijn eigen `skeletonHtml()`, zoals Bewaartermijnen en Plaatsingen dat deden; `index.html` draagt alleen het lege `<section>` en het sidebar-item. Drie dingen zijn paneelbreed (`js/ui.js`), niet alleen voor deze sectie: `ui.modal`'s `confirmCaseInsensitive` en `confirmInputType`-opties en `handle.confirmValue()`, voor de enige destructieve modal in dit paneel waarvan de getypte bevestiging geen vaste tekenreeks is maar het adres zelf (zie de as-builtaanvulling hierboven); `data-gsp-role` op elke paneelknop, zodat een knop die client-side van klasse wisselt (punt 2 hierboven) toch los te selecteren blijft; en `close()` die het paneel-element leegmaakt na het verbergen (punt 6 hierboven, security-audit), voor elke modal en drawer in het paneel, niet alleen deze twee. Verder geen nieuwe `.a-`-klassen: de kaartlijst, het panel en de metric-rows hergebruiken `.a-cardlist`, `.a-cell-name`, `.a-num`, `.a-soft`, `.a-panel` en `.a-metric-row` zoals Bewaartermijnen en Plaatsingen die al gebruiken. `#section-gdpr .btn` is toegevoegd aan de bestaande 44px-tikdoelregel op 390px, naast `#section-retention`/`#section-candidates`/`#section-placements`. Geen nieuwe labelmap in `js/labels.js`: dit scherm toont geen enumwaarde, alleen domein, reden (vrije tekst) en een hash.
