...
De acht kaartarchetypen zelf zijn **icoonloos** (A's kernprincipe: Newsreader-cijfers of Plex Mono 2-lettercodes in een `.mark`-kader in plaats van een pictogram — dit lost het bestaande "drie icoonstijlen door elkaar"-probleem op zonder een vierde stijl toe te voegen, en is immuun voor icon-blokkerende adblockers). Buiten de kaarten staat de site vol met niet-kaart-iconen (filters, formuliervalidatie, footer-links, statusmeldingen) die vandaag nog Font Awesome zijn. Voor die resterende iconen komt één same-origin sprite, `website/icons.svg`: een verborgen `<svg style="display:none">` met een `<symbol id="icon-*">` per pictogram, 24×24-raster, `stroke-width:1.5`, `stroke:currentColor`, `fill:none`, `stroke-linecap/linejoin:round`, aangeroepen via `<use href="#icon-*">`. Elk icoon krijgt `aria-hidden="true"`; het label ernaast draagt de betekenis.

**Uitzondering, tot fase 2**: Font Awesome blijft in gebruik voor **header, login/registratie-modal en back-to-top-knop** — deze drie zijn chrome-elementen buiten de scope van deze kaartmigratie en worden apart gepland.

~24 iconen in de eerste sprite:
...
| `paper-plane` | verzendknoppen (contactformulier, CV-upload) — vervangt `fa-paper-plane` uit de huidige lege-vacaturestaat (`script.js:681`) |

---

#### 8.x.4 Contactrail (sitebreed, vervangt `.whatsapp-float` en `.mail-float`)

Vervangt de groene pulserende WhatsApp-bubbel (`#25D366`, `website/styles.css:1826-1858`) en de losse navy mail-knop (`1860-1885`) die vandaag op elke pagina zweven. Nieuw: één navy pil, radius `var(--radius)` (3px — ondanks de naam "pil" géén `--radius-full`, dat blijft gereserveerd voor chips), samengesteld uit twee gestapelde items van elk 48×48px (WhatsApp, e-mail), mono-label wit op navy als glyph (`WA` / `@`, consistent met A's `.mark.code`-systeem — geen icoon, geen kleur buiten navy/wit, geen puls-animatie, nooit `#25D366`).
...
- **390px**: zelfde compacte 48×48-items, geen verkleining (in tegenstelling tot de huidige `.mail-float`/`.whatsapp-float`, die op mobiel al 48px zijn maar op verschillende hoeken staan). De uitklap-tekst blijft desktop-only progressive enhancement; op touch is het glyph zelf het enige zichtbare label, met de `aria-label` als schermlezer-vangnet.
...
- **NL**: kop "Op dit moment werven we nieuwe rollen", body "Meld je aan voor de talentpool, dan nemen wij contact op zodra een passende rol binnenkomt.", CTA "Meld je aan voor de talentpool →".
- **EN**: kop "We are not actively hiring for new roles right now", body "Join the talent pool and we will reach out as soon as a suitable role comes in.", CTA "Join the talent pool →".
