...
**As-builtaanvulling op §7.3.3.** "Bewerken" staat in de sticky voettekst van de plaatsingendrawer (punt 2 hierboven), niet in de tab Financieel: het bewerkmodal wijzigt ook Overzicht-velden (start-/einddatum, notities), niet alleen financiële velden. Verwijderen is alleen bereikbaar vanuit de rij in de lijst (de rijknop met het prullenbakicoon), niet vanuit de drawer, en die knop is `.a-hide-cardlist` -- op 390px dus niet aanwezig; een plaatsing verwijderen op een telefoon gaat via de brede weergave. Een bedrag met een duizendtalscheiding zoals `1.234,56` wordt afgewezen met de foutmelding "Gebruik één decimaalteken": de parser accepteert komma óf punt als decimaalteken, nooit allebei tegelijk in hetzelfde getal.
