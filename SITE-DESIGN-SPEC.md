...
> **Status, september 2026: gebouwd**, als `website/admin/js/sections/candidates.js`. De kandidaatdrawer zelf (Profiel, Matches, Activiteit, Toestemmingen) is met deze PR gebouwd, niet alleen de tab Toestemmingen: er bestond voordien geen drawer, alleen een brede modal met één paneel. Alles hieronder staat er, met drie afwijkingen die hier ter plekke genoemd worden:
> 1. **Toestemmingen (en de twee wijzigmodals) werken op `candidates.id`, niet op het kind/id-paar waarmee de lijst en de drawer een kandidaat aanspreken.** Een self-registered kandidaat zonder gekoppelde `candidates`-rij heeft geen grondslag om een van de drie toestemmingen op vast te leggen: `candidateRecordId()` leest `.id` (kind `sourced`) of `.candidate_id` (kind `self-registered`) uit de al opgehaalde detailrespons, en ontbreekt dat, dan toont de tab één zin ("Deze kandidaat heeft nog geen kandidaatrecord; toestemmingen zijn hier niet beschikbaar.") in plaats van de drie kaarten uit de wireframe. "Leeg bestaat niet" blijft waar voor een kandidaat mét een `candidates`-rij; dit is de ene voorwaarde ervoor.
> 2. **Matches en Activiteit hergebruiken bestaande, gedeelde routes** in plaats van een eigen matcheslijst: Matches leest `GET /api/v1/admin/pipeline?candidate_id=` (BV1, §7.3.4) en toont vacature, fase en laatst gewijzigd; Activiteit leest `GET /api/v1/admin/activities?subject_type=candidate&subject_id=` zoals §7.3.6(b) voor de klantdrawer al doet. Er is geen aparte "matches"-route voor één kandidaat en de pipeline-rijvorm is inhoudelijk hetzelfde ding.
> 3. **De titel bij een actieve presentatietoestemming ("`<titel>` · Vacature #`<id>`") is best effort.** Er is geen admin-route voor één losse vacature. De tab haalt bij het openen, alleen wanneer er een `consent_spec_presentation_job_id` is, eenmalig de volledige (ongefilterde) vacaturelijst op (`GET /api/v1/admin/jobs?limit=200`) om de titel te vinden; staat de vacature daar niet in (verwijderd, of buiten de eerste 200), dan toont de tab alleen "Vacature #`<id>`" zonder titel.
>
> De radiogroep in beide wijzigmodals staat bij openen altijd op "Vastleggen", ook wanneer de toestemming al actief is: de spec schrijft geen standaardkeuze voor, en "vastleggen/vernieuwen" is de handeling waarvoor een beheerder de knop "Wijzigen" (talentpool) of "Vastleggen" (presentatie) klikt. De datums en het bewijsveld gebruiken `Admin.retentionDate()` uit §7.3.1 (Nederlandse maandafkortingen) in plaats van `Admin.formatDate()` (en-GB), zodat de wireframe-datum "3 sep 2026" ook zo op het scherm staat. `--tblr-form-invalid-color`/`--tblr-form-invalid-border-color` stonden in de gevendorde Tabler-CSS hardcoded op `#d63939` in plaats van afgeleid van `--tblr-red`; zonder een eigen regel in `admin.css` had elk `.invalid-feedback`/`.is-invalid`-veld in dit paneel dus buiten de merktokens om gekleurd, niet alleen in deze tab. Bewaakt door `scripts/admin_sections_check.py` (de drie modals en de drie referral-uitkomsten tegen gestubde routes).
