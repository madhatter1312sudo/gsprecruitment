...
>
> Bewaakt door `scripts/admin_sections_check.py`: zes gestubde pipeline-entries op candidate_id=1482/client_id=1 (waarvan vijf op de kandidaat, voor de accordeon-met-vijf-entries-test, en zes op de opdrachtgever, plus een zesde kandidaat voor de naam-in-de-kop), het exacte zeven-opties-in-spec-volgorde-contract van de select, de "(bestaande waarde)"-optie voor zowel een onbekende als een lege fase die geen van beide ooit verstuurd wordt, één PATCH met de juiste payload gevolgd door precies één verse `GET .../history`-aanroep (niet lokaal aangevuld, niet alleen "de tekst staat er al ergens"), een 422 gevolgd door een 500 gevolgd door een geslaagde poging op dezelfde entry (optimistische update, terugdraaien, dan alsnog slagen), de drie actor-varianten in de historie (genoemd, "Gebruiker #`<id>`", "Onbekend") en het `(nieuw)`-label bij `from_stage: null`, "Bijgewerkt" dat blijft staan zonder historie en mét een historiefout, twaalf historie-items met "Toon alles", een entry waarvan de historieroute altijd 500 teruggeeft terwijl de fasewisselaar zelf bruikbaar blijft, en de kruisdrawer-regressie hierboven.
