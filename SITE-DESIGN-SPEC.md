...
>
> Bewaakt door `scripts/admin_sections_check.py`: vijf gestubde pipeline-entries (waarvan vier op één kandidaat, voor de accordeon-met-vier-entries-test in de kandidaatdrawer, en vijf op één opdrachtgever voor dezelfde test in de klantdrawer plus de kandidaatnaam-in-de-kop), het exacte zeven-opties-in-spec-volgorde-contract van de select, de "(bestaande waarde)"-optie die nooit verstuurd wordt, één PATCH met de juiste payload gevolgd door een verse `GET .../history` (niet lokaal aangevuld), een 422 gevolgd door een 500 gevolgd door een geslaagde poging op dezelfde entry (optimistische update, terugdraaien, dan alsnog slagen), de drie actor-varianten in de historie (genoemd, "Gebruiker #`<id>`", "Onbekend") en het `(nieuw)`-label bij `from_stage: null`, twaalf historie-items met "Toon alles", en een entry waarvan de historieroute altijd 500 teruggeeft terwijl de fasewisselaar zelf bruikbaar blijft.
