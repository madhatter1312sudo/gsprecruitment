...
Vervangt de groene pulserende WhatsApp-bubbel (`#25D366`, `website/styles.css:1826-1858`) en de losse navy mail-knop (`1860-1885`) die vandaag op elke pagina zweven. Nieuw: één navy pil, radius `var(--radius)` (3px — ondanks de naam "pil" géén `--radius-full`, dat blijft gereserveerd voor chips), samengesteld uit twee items van elk 48×48px (WhatsApp, e-mail), gestapeld op ≥601px en naast elkaar op ≤600px (zie 390px hieronder), mono-label wit op navy als glyph (`WA` / `@`, consistent met A's `.mark.code`-systeem — geen icoon, geen kleur buiten navy/wit, geen puls-animatie, nooit `#25D366`). **Rand + schaduw (reparatie, chief-of-staff review, niet in het oorspronkelijke ontwerp):** `border: 1px solid var(--navy-600)` plus `box-shadow: var(--shadow-md)`. Zonder eigen rand versmelt de pil op de navy hero/footer met de achtergrond erachter (die vlakken zijn dezelfde kleur) en is niet als knop herkenbaar; dit is dus geen decoratieve schaduw maar hetzelfde elevatie-signaal als de §8.x.0-uitzondering.

- **Positie**: `position:fixed; bottom:var(--space-lg); z-index:9995`. Op >600px linksonder (`left:var(--space-lg)`), op <=600px rechtsonder (`right:var(--space-lg); left:auto`). Op smalle schermen begint links elke regel lopende tekst, dus daar dekt de pil hele woorden af; rechts eindigen de regels rafelig, zodat de pil daar in de witruimte valt. Gemeten in Chromium (index, kandidaten, werkgevers, vacatures, contact; 390x844 en 390x664; NL en EN; scrollpositie 0 en onderaan de pagina, 40 metingen): linksonder 13.148 px2 afgedekte lopende tekst, rechtsonder 4.113 px2. Onderaan de pagina is de overlap in beide opstellingen 0 px2 (dat regelt de `padding-bottom` op `.footer`, zie hieronder).
- **Vaste-elementenstapel op <=600px**: afstanden vanaf de onderrand van het scherm, gemeten in dezelfde run. Contactrail 24-74px; `.back-to-top` 80-122px (de CSS-variant op index/over-ons/werkwijze/404) respectievelijk 80-124px (de variant die `script.js` injecteert op de overige pagina's); `.toast-container` vanaf 132px. De vrije ruimte tussen de zichtbare back-to-top en de rail is 6px; in de onzichtbare ruststand schuift de back-to-top met een `translateY` naar beneden en raakt hij de rail over 160-340 px2, maar die staat dan op `opacity:0` en `pointer-events:none`, dus er is geen tikdoel dat verschuift. De toastcontainer stond eerder op `bottom:24px` en legde zich met `z-index:9998` over de rail (2.238 px2 gemeten op elke pagina waar een toast kan verschijnen, in beide hoeken); op <=600px staat hij nu boven de hele stapel, gemeten overlap 0 px2.
...
- **Toetsenbordbediening**: de pil is een `<nav aria-label="Direct contact">` met twee echte `<a href="https://wa.me/...">`/`<a href="mailto:...">`-elementen erin, elk een natuurlijke tab-stop. Focus-visible toont dezelfde uitgeklapte tekst als hover — geen apart, onzichtbaar toetsenbordpad.
- **390px**: zelfde 48x48-tikdoel per item, geen verkleining; de twee items staan hier naast elkaar (`flex-direction:row`) in plaats van gestapeld, wat het footprint van 48x96 naar 96x48 brengt. Beide kanalen blijven bereikbaar. **Restrisico, met meetwaarden:** een element dat de volle contentbreedte beslaat, kan nog steeds onder de pil vallen, ongeacht de hoek. De 4.113 px2 die na de verplaatsing overblijft, zit in vier van de 40 gemeten combinaties: het CTA-knoplabel op werkgevers EN 390x664 (1.366 px2), het talentpool-checkboxlabel op kandidaten EN 390x844 (2.194 px2), de herochip "KvK 75545586" op werkgevers EN 390x844 (348 px2) en het laatste woord van de heroparagraaf op index EN 390x844 (205 px2). In alle NL-metingen en op elke scrollpositie onderaan de pagina is de overlap 0 px2. Een verdere reductie (een kanaal weglaten, of de pil pas tonen na een scroll-drempel) is niet doorgevoerd: dat verandert welke kanalen bereikbaar zijn respectievelijk voegt scroll-logica toe, en is een ui-designer-beslissing. De uitklap-tekst blijft desktop-only progressive enhancement; op touch is het glyph zelf het enige zichtbare label, met de `aria-label` als schermlezer-vangnet.
- **Footer-botsing**: `.footer` heeft op <=600px `padding-bottom: var(--space-5xl)` (128px), zodat de laatste footerregel (copyright/KvK/AVG) bij elke scrollpositie boven de pil blijft. Dit geldt voor elke pagina met de gedeelde footer en is geen scroll-afhankelijk lapmiddel; gemeten overlap onderaan elke pagina is 0 px2.
