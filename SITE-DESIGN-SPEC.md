...
- **Positie**: `position:fixed; left:var(--space-lg); bottom:var(--space-lg); z-index:9995`. Bewust **linksonder**, niet rechtsonder: op 390px staan daar al drie vaste elementen (`.back-to-top` op `bottom:96px;right:24px`, `.toast-container` op `bottom:24px;right:24px`, `.cookie-banner` volle breedte onderaan) en elke rechtsonder-positie botst met minstens twee daarvan. Linksonder is vrij.
...
- **390px (herzien, chief-of-staff review):** zelfde 48×48-tikdoel per item, geen verkleining (in tegenstelling tot de huidige `.mail-float`/`.whatsapp-float`, die op mobiel al 48px zijn maar op verschillende hoeken staan); de twee items staan hier echter **naast elkaar** (`flex-direction:row`) in plaats van gestapeld: de gestapelde 48×96px-vorm viel op smalle schermen over de laatste tekstregel(s) van de content eronder (de kaarten in de hero, de footer); de 48×48px-brede vorm halveert dat footprint. Restrisico, expliciet benoemd in plaats van verzwegen: bij een ongelukkige scrollpositie kan de pil nog de allerlaatste regel van een kaartbeschrijving raken; de kaart-CTA (`.go-link`, altijd `margin-top:auto` onderaan) blijft in de geverifieerde gevallen vrij. Een verdere reductie (één kanaal op mobiel, of de pil pas tonen na een scroll-drempel) is niet doorgevoerd, want dat verandert welke kanalen bereikbaar zijn resp. voegt scroll-logica toe, en is een expliciete ui-designer-beslissing, geen frontend-fix. De uitklap-tekst blijft desktop-only progressive enhancement; op touch is het glyph zelf het enige zichtbare label, met de `aria-label` als schermlezer-vangnet.
- **Footer-botsing (reparatie, chief-of-staff review):** aan het eind van elke pagina viel de pil op ≤600px exact over de copyright-/KvK-regel (`WA` botste met `AVG`), omdat die regel op precies dezelfde schermpositie eindigde. `.footer` krijgt op ≤600px `padding-bottom: var(--space-5xl)` (128px) zodat de laatste footerregel bij elke scrollpositie boven de pil blijft; dit lost de botsing structureel op (voor élke pagina met de gedeelde footer), niet als scroll-afhankelijke lapmiddel.
