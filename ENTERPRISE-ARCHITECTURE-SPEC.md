...
Auth router (`routers/auth.py`) covers: register, login, JWT refresh, email verification (tokens: `secrets.token_urlsafe`, only their SHA-256 hash stored, 24h TTL), password reset/forgot, change/set password, profile read/update, and Google sign-in. There is no LinkedIn OAuth backend flow and no LinkedIn button in the UI anymore, the button was never wired to a token exchange, so it was removed.

**Google sign-in flow** (`GET /api/auth/google/login`, `GET /api/auth/google/callback`, same Google Cloud OAuth client already configured for transactional email):

- `login?role=candidate|client&next=/pad` starts the flow. `role` defaults to `candidate`; `next` is optional, must start with a single `/`, must not start with `//`, and is capped at 200 characters, anything else is ignored. The endpoint packs `role`, `next` and a nonce into a JWT valid for 10 minutes and sends it to Google as `state`; the nonce is also set in a `google_oauth_state` cookie so the callback can confirm the state came from this browser. Missing OAuth configuration redirects to `{FRONTEND_URL}/?google_auth_error=not_configured` instead of a bare JSON 503.
- `callback` verifies `state` against the cookie, exchanges the code, and reads the Google profile. An existing user keeps their stored role, the requested `role` is ignored for them; a new user is created with the requested role, `role=client` runs through the same account-creation branch as `register()` (factored into a `_create_client_account` helper), anything else creates a candidate. On success it redirects to `{FRONTEND_URL}{next}#google_auth=<jwt>`, but only when `next` matches the resulting role, otherwise `/` for a candidate or `/client/` for a client.
- Failures redirect to `{FRONTEND_URL}/?google_auth_error=<code>` with one of `not_configured`, `invalid_state`, `missing_code`, `token_exchange_failed`, `email_not_verified`, `account_disabled`, `admin_use_password`, `server_error`, or Google's own `access_denied` passed through unchanged (nine codes in total). The frontend (`website/script.js`) maps each code to a short NL/EN toast and strips the parameter from the URL right after reading it, so reloading the page does not repeat the message.
...
- **Retention table**: single source of truth is `talent-os/backend/core/retention.py`; `docs/VERWERKINGSREGISTER.md` §1.4 and `docs/SOURCING-SOP.md` are generated copies of the same table, checked against the code by `tests/test_retention.py`. Nothing deletes automatically any more (owner decision, WS-E.10): a monthly job (`services/scheduler.py::generate_retention_review()`, apscheduler, 1st of month 04:00 Europe/Amsterdam) queues who each row's guarded selector says is due into `retention_review_items`, with the category, the date the term expired, and which protective signal was absent. An admin approves (`POST /api/v1/admin/retention/review/{id}/approve`, requires `confirm:"APPROVE"`, re-checked for eligibility right before acting) or rejects (`POST .../review/{id}/reject`, no confirm needed) each item; only approval triggers the actual `erase_person` logic or hard delete. `POST /api/v1/admin/retention/run` is counts-only now, always. The former separate Apollo-pool purge endpoint (`POST /api/v1/admin/apollo-pool/purge`) shares this same queue (`category='apollo_pool_purge'`) and now only returns a dry-run preview.
