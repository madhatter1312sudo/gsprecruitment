...
|---|---|---|
| `GET /v1/mobile/me/matches`, `GET /v1/mobile/me/applications` | Both now exist (`routers/mobile.py`), but so do `GET /api/v1/candidate/matches`, `GET /api/v1/candidate/applications` (`routers/candidate.py`), which is what this app was originally wired to | **Decision: keep `/v1/candidate/...`.** The mobile variants return a different shape (unpaginated `{items}`, renamed fields — `match_id`/`score`/`stage` instead of `id`/`match_score`/`status`), so adopting them would mean rewriting `MatchItem`, both query hooks and the matches/applications screens for zero user-visible benefit. The candidate endpoints also paginate, which the mobile ones don't. |
...
| `POST /v1/mobile/push-token` (+ `DELETE` for logout/uninstall) | **Now live** in production — but it only **stores** tokens: nothing in the backend reads `push_tokens` besides the router itself and migration `012_mobile_growth.py`. There is no Expo-push/FCM/APNs sender pipeline yet, so registered tokens would go nowhere. | **Decision: push registration stays deferred.** `lib/api.ts`'s `registerPushToken()` points at the real endpoint but is never called. Wiring it up requires (a) `expo-notifications` (not in the approved minimal dependency list), (b) a development build — remote push doesn't work in Expo Go since SDK 53 — and (c) FCM/APNs or Expo push credentials. None of that pays off until the backend actually sends notifications. When a sender pipeline lands: add `expo-notifications`, register the token after login/session restore, and call `DELETE /v1/mobile/push-token` on logout. |
